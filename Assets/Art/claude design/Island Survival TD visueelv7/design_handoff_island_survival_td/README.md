...
| `sprites/` | **94 kant-en-klare PNG's** — direct importeerbaar in Unity |
| `sprites/README.md` | pivots, framevolgorde, speelsnelheden, importinstellingen |
| `WATER.md` | losse spec voor de watertegels — 4 varianten, distributie, tiling, framerate, palet |
...
### Water
De watertegels zijn af en zitten in `sprites/` — zie **`WATER.md`** voor de volledige spec
(bestandsnamen, distributie, VRAM-afweging, importinstellingen, tiling, framerate, palet). Kort:
**4 varianten van 1024×1024, elk 12 frames**, elk apart naadloos in x en y, basis `#456DF5` met een rond
celnetwerk van 1px lichte lijnen en witte schuimvlekjes. Eén frame per 0,15s, alle tegels op
dezelfde frame-index, één gedeelde timer. Wijs per grid-cel willekeurig een variant toe (hash op
celcoördinaat), zoals de grastegels al verdeeld worden. Op dit formaat kost de set wel 48 MB VRAM
per variant — `WATER.md` legt uit waarom één of twee varianten meestal genoeg is.

Let op: dit vervangt de eerdere donkere zee (`#14496B` / `#4FC9D9` / `#A9EEF2`). Die kleuren
staan nog wel in het palet omdat ramen, zeilen en de UI ze gebruiken, maar niet meer als water.
De bootsprites hebben nog schuim- en kielzoglijnen in `#A9EEF2`, geauthord tegen de oude zee —
die vragen een recolor-pass.
...
- **Animaties in-world**: gebruik de kant-en-klare frames uit `sprites/` — boot-deining 4 frames op ~6 fps, water 12 frames op ~7 fps (0,15s per frame), alle varianten in fase, gras 4 frames op ~5 fps, toren-vuur 3 frames op ~20 fps als one-shot. Schoorsteenrook op het dorp: 2 frames / 1.2s. Alles op hele pixels — nooit sub-pixel bewegen. Geef elke boot en elke grastegel een willekeurige start-offset zodat ze niet in lockstep bewegen.
...
94 kant-en-klare PNG's, afgeleid van de definitieve sprites: toren-lagen (base + roteerbare
head + 3-frame vuur-animatie per toren), een naadloos tegelbare geanimeerde watertegels (4 × 1024×1024,
12 frames), deining voor de 4 piratenboten (4 frames elk) en verrijkte grastegels met
