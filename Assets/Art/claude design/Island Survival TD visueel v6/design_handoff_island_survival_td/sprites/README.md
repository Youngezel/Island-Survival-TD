...
## 2. Water — `water_wave_01..12.png`

64×64, **12 frames**, naadloos herhaalbaar in **beide** richtingen (x en y).

Basis is helder koningsblauw `#456DF5`. Daarover een celnetwerk (voronoi met 6 Lloyd-relaxatie-
stappen, dus ronde, gelijkmatig verdeelde cellen van ~8px in plaats van hoekige scherven). De
lijnen lopen in vier stappen over in het water: `#80A2E8` kern → `#6A8FEE` → `#567DF3` → basis,
met sporadisch `#AAC3F1` als lichte glinstering. Ongeveer 10% van de cellen is iets donkerder
(`#3F65EF`); ~22 witte schuimvlekjes van 1–2px liggen verspreid over het vlak.

Animatie: de celzaden draaien in een cirkeltje van 0,18–0,4px en lopen exact rond over 12 frames.
Verschil tussen opeenvolgende frames is max 5,9% van de pixels — het netwerk kruipt rustig,
zonder te koken. Aanbevolen snelheid: ~1,8s per lus (≈7fps).
...
58 PNG's in totaal: 15 toren-lagen, 12 waterframes, 16 bootframes, 15 grastegels.
