...
## Overzicht
Complete visuele identiteit voor een 2D hex-grid tower defense game in Unity: verdedig een dorpje op een tropisch eiland tegen golven piraten. Dit pakket bevat het kleurenpalet, typografie, sprite-specificaties (4 vijandtypes, dorp, 3 torens, hex-tegels) en pixel-exacte mockups van alle vier UI-schermen (HUD, hoofdmenu, wave-keuze, game over).

## Over de design-bestanden
Het bestand `Island Survival TD - Visuele Identiteit.dc.html` is een **design-referentie in HTML** — een prototype dat toont hoe het spel eruit moet zien. Het is **geen productiecode om te kopiëren**. De opdracht is: dit ontwerp implementeren in het bestaande Unity-project met Unity's eigen middelen (sprites, Tilemap, uGUI/Canvas of UI Toolkit). De HTML-blokjes zijn een tekening van de sprites, geen assets.

Alle sprites in de HTML zijn met CSS-rechthoeken nagebouwd. Ze dienen als **sprite-briefing**: silhouet, opbouw en kleurvlakken per unit. De uiteindelijke .png-sprites moeten door een pixel-artist (of via deze specs) getekend worden.

## Fidelity
Gemengd — lees dit voordat je begint:

- **Hi-fi: de UI-schermen** (HUD, hoofdmenu, wave-keuze, game over). Ontworpen op de native cameraresolutie 640×360 en in de HTML 2× vergroot getoond. Alle posities, groottes, kleuren en teksten in dit document zijn in **native pixels** en moeten pixel-exact worden overgenomen.
- **Lo-fi: de in-world sprites** (boten, dorp, torens, tegels). De maten, kleurvlakken en silhouet-regels zijn bindend; de precieze pixelplaatsing is aan de artist. Elke boot heeft in de HTML ook een **zwarte silhouet-variant** — dat is de leesbaarheidstest: als het silhouet niet direct van de andere drie te onderscheiden is, is de sprite fout.
...
- **Animaties in-world**: boot-deining 2 frames / 0.6s; schoorsteenrook 2 frames / 1.2s; water-golflijnen scrollen horizontaal ~4 px/s. Alles op hele pixels — nooit sub-pixel bewegen.
