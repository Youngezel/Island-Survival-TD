...
## Tesla Coil — upgrade heads (toevoeging)

De Tesla Coil is geen kanon: er is geen loop en geen richtbare voorkant. Elke head is
radiaal symmetrisch rond de pivot, zodat de bestaande rotatiecode het sprite mag blijven
draaien zonder dat het opvalt.

- Pivot: **16, 20** — gelijk aan de andere turret-heads, dus geen codewijziging nodig.
- Alle getekende pixels liggen binnen radius 11 van de pivot; roteren clipt nooit.
- Shading loopt radiaal (binnen licht, buiten donker) in plaats van licht-van-links,
  zodat de lichtrichting niet meedraait.
- Flash is een overlay bovenop de head, in dezelfde rotatie. F1 -> F2, ~60 ms per frame.
- Geen projectielsprites: de hit is de bestaande LightningBolt.
- `spr_turret_tesla_coil_base.png` blijft ongewijzigd.

### Pad A — Overspanning / Reikwijdte
| Tier | Upgrade | Silhouet |
|---|---|---|
| 1 | Meer Damage | stalen toroid r=6, 4 geleiders met cyaan tip, koperen naaf |
| 2 | Ricochet | zwaardere toroid r=8 + binnenring, 4 lange geleiders naar r=11 + 4 korte |
| 3 | Meer Range | dubbele toroid r=9 en r=5, 8 geleiders naar r=11, wit-cyaan kern |

### Pad B — Overload
| Tier | Upgrade | Silhouet |
|---|---|---|
| 1 | Meer Doelen | losse windingen, 6 bogen naar buiten, koperen naaf nog zichtbaar |
| 2 | Meer Damage | behuizing verdonkerd, 9 bogen + gebroken veldring, wit-hete kern |
| 3 | Overload | behuizing vrijwel weg, 12 bogen, paars/wit-heet corona |

### Bestanden
```
spr_tesla_coil_head_a_t1.png   spr_tesla_coil_flash_a_t1_f1.png  spr_tesla_coil_flash_a_t1_f2.png
spr_tesla_coil_head_a_t2.png   spr_tesla_coil_flash_a_t2_f1.png  spr_tesla_coil_flash_a_t2_f2.png
spr_tesla_coil_head_a_t3.png   spr_tesla_coil_flash_a_t3_f1.png  spr_tesla_coil_flash_a_t3_f2.png
spr_tesla_coil_head_b_t1.png   spr_tesla_coil_flash_b_t1_f1.png  spr_tesla_coil_flash_b_t1_f2.png
spr_tesla_coil_head_b_t2.png   spr_tesla_coil_flash_b_t2_f1.png  spr_tesla_coil_flash_b_t2_f2.png
spr_tesla_coil_head_b_t3.png   spr_tesla_coil_flash_b_t3_f1.png  spr_tesla_coil_flash_b_t3_f2.png
```

Nieuwe kleuren t.o.v. de bestaande set: energy `1F5F8F / 3F8FC4 / 7FD4F0 / DFF3FF`
en overload `7A3FB0 / B07CE8 / E8D9FF / FFFFFF` (alleen B-T3).

### Tier 0 — standaard kop + coil-voet

- `spr_turret_tesla_coil_head.png` — 32x32, pivot **16, 20**. De kleinste toroid in de reeks:
  stalen ring r=4 (buitenrand donker, binnenrand licht), 4 korte geleiders met cyaan tip,
  koperen naaf met een cyaan kern. Radiaal licht, dus rotatie-neutraal zoals de tier-koppen.
- `spr_turret_tesla_coil_base.png` — 32x32, pivot bottom-center (0.5, 0), roteert niet.
  Van boven af getekend, net als de rest van het bord: concentrische ellipsen rond hetzelfde
  middelpunt als de kop-pivot (16, 20), licht y-geplet (0.85) voor een lichte kanteling.
  Van buiten naar binnen: donkere grondrand, twee koperen primary-windingen met een donkere
  scheiding ertussen, 4 radiale stalen steunen, stalen strike ring, en de koperen secondary
  wikkelcilinder in bovenaanzicht.
  Licht van linksboven — de voet roteert niet, dus daar geldt de radiale-licht-regel niet.
- De kop-toroid dekt precies de secondary af, zoals de topload in de foto. De strike ring en
  beide primary-windingen blijven zichtbaar onder elke kop, ook onder de brede tier-3 koppen.

### Centrering (belangrijk voor de rotatie)

Alle Tesla Coil-sprites zijn opnieuw getekend zodat hun massa exact om het rasterpunt
(16, 20) ligt — dus om de pivot zelf, niet om het midden van pixel 16/20. Dat verschil
van een halve pixel was zichtbaar als een lichte slinger tijdens het draaien.

- Bounding box van elke head, elk flashframe en de base: middelpunt exact 16.0 / 20.0
  (gemeten, afwijking 0 px).
- Pad A is 4- of 8-voudig spiegelsymmetrisch.
- Pad B is nu rotatiesymmetrisch in plaats van willekeurig: er is één boog getekend die
  6x (T1), 8x (T2) of 12x (T3) rond het middelpunt gestempeld wordt. Chaotisch van vorm,
  maar perfect in balans rond de pivot.
- Flashframes volgen dezelfde n-voudige symmetrie als de head waar ze bij horen.
- De base is om hetzelfde punt gecentreerd, zodat elke kop precies in het midden van de
  spoel staat.
